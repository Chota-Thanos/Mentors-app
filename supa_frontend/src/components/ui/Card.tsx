...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={`rounded-xl border border-slate-200 bg-white font-sans text-slate-950 shadow-sm dark:border-slate-800 dark:bg-[#0f172a] dark:text-slate-50 ${className || ""}`}
      {...props}
    />
  )
)
